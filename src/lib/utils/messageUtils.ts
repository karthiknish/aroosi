...
export function groupMessagesByDate(messages: MessageData[]): Record<string, MessageData[]> {
...
export function shouldShowTimestamp(currentMessage: MessageData, previousMessage?: MessageData): boolean {
  ...
  return minutesDiff > 15 || currentMessage.fromUserId !== previousMessage.fromUserId;
}

// Typing indicator utilities
const typingTimeouts = new Map<string, NodeJS.Timeout>();

export function handleTypingStart(conversationId: string, userId: string, onTypingUpdate: (typing: TypingIndicator) => void): void {
...
export function handleTypingStop(conversationId: string, userId: string, onTypingUpdate: (typing: TypingIndicator) => void): void {
  const existingTimeout = typingTimeouts.get(`${conversationId}_${userId}`);
  if (existingTimeout) {
    clearTimeout(existingTimeout);
    typingTimeouts.delete(`${conversationId}_${userId}`);
...
export function calculateUnreadCount(messages: MessageData[], currentUserId: string): number {
...
export function searchMessages(messages: MessageData[], query: string): MessageData[] {
...
export function filterMessagesByType(messages: MessageData[], type: 'text' | 'voice' | 'image'): MessageData[] {
  return messages.filter(message => message.type === type);
}

// Message export utilities
export function exportConversationToText(messages: MessageData[], userNames: Record<string, string>): string {
