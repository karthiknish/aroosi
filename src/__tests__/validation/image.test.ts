...
describe("validateImageUpload", () => {
  ...
  it("detects signature mismatch (jpeg bytes but png mime)", () => {
    ...
    // Might not always detect due to simplistic end marker requirement but we ensure mismatch path works.
    if (!res.ok) {
      expect(["SIGNATURE_MISMATCH","MIME_DISALLOWED"]).toContain(res.errorCode);
    } else {
      // If accepted, at least ensure allowed list contains mime
