     ...
     const resp = await new Promise<Response>((resolve, reject) => {
       try {
         ...
         xhr.onload = () => {
           ...
           headers.set("Content-Type", xhr.getResponseHeader("Content-Type") || "text/plain");
 ...
 */
export function safeNavigate(router: { push: (p: string) => void }, href: string) {
  try {
    router.push(href);
  } catch (e) {
 ...
 */
export function formatMinorUnitPrice(minor: number, currency: string = "GBP", locale?: string) {
  try {
...
export function focusFirstErrorField(
  ...
  const anyInvalid = document.querySelector('[aria-invalid="true"]') as HTMLElement | null;
 ...
 */
export function validateBlobSize(blob: Blob, maxBytes: number): { ok: true } | { ok: false; reason: string } {
  if (blob.size > maxBytes) {
    return { ok: false, reason: `Image exceeds ${(maxBytes / (1024 * 1024)).toFixed(0)}MB` };
  }
  return { ok: true };
}
 ...
 */
export function deriveSafeImageMimeType(type: string | null | undefined): string {
  const allowed = ["image/jpeg", "image/jpg", "image/png", "image/webp"];
  if (typeof type === "string" && allowed.includes(type.toLowerCase())) return type;
  return "image/jpeg";
}
