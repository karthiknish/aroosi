...
export default function AdminMatchesPage() {
  ...
  const totalMatches = matches.reduce(
    (sum, item) =>
      sum + (Array.isArray(item.matches) ? item.matches.length : 0),
    0
  );
        ...
        <>
          {matches.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {matches.map((item) => (
                <Card
                  key={String(item?.profileId || Math.random())}
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-lg truncate">
                        {item?.profileId
                          ? `Profile ${String(item.profileId).slice(-8)}`
                          : "Profile"}
                      </CardTitle>
                    ...
                    item.matches.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {item.matches.map((match) => {
                          const age = getAge(match.dateOfBirth as any);
                          const matchImages =
                            profileImages[match._id as any] || [];
                          const imageUrl =
                            matchImages.length > 0 ? matchImages[0].url : null;

                          return (
                            <div
                              key={match._id as any}
                            ...
                            >
                              ...
                              <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-sm flex-shrink-0">
                                {imageUrl ? (
                                  <img
                                    src={imageUrl}
                                    alt={(match.fullName as any) || "Profile"}
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="font-semibold text-sm truncate">
                                  {(match.fullName as any) || "Unnamed"}
                                </div>
                                <div className="text-xs text-gray-500 flex items-center gap-2">
                                  <span>{(match.city as any) || ""}</span>
                                  {age && <span>• Age {age}</span>}
                                </div>
                                <div className="text-xs text-gray-400 truncate">
                                  {(match.occupation as any) || ""}
                                </div>
                              </div>

                              {/* Action Button */}
                              <Link href={`/admin/profile/${match._id as any}`}>
                                <Button
                                  size="icon"
                                  variant="ghost"
