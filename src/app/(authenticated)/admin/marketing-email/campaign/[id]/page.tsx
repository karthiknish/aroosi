    ...
    <Card className="max-w-5xl">
      ...
      <CardContent className="space-y-6">
        ...
        {summary?.trackingByHour && (
          <div className="space-y-2">
            {(["opens", "clicks"] as const).map((kind) => {
              ...
              const maxVal = Math.max(1, ...entries.map(([, v]) => v as number));
              return (
                <div key={kind}>
                  <div className="text-xs mb-1 capitalize">{kind}</div>
                  <div className="flex items-end gap-1 h-12">
                    {entries.map(([k, v]) => (
                      <div key={k} title={`${k}: ${v}`} className="bg-blue-500" style={{ width: 4, height: Math.max(2, Math.round((Number(v) / maxVal) * 48)) }} />
                    ))}
                    {entries.length === 0 && <div className="text-xs text-muted-foreground">No {kind} yet</div>}
                  </div>
                </div>
              );
            })}
            ...
            <tbody>
              {emails.map((it) => (
                <tr key={it.id} className="border-t">
                  ...
                  <td className="p-2">{Array.isArray(it.to) ? it.to.join(", ") : it.to}</td>
              ...
              {emails.length === 0 && (
                <tr>
                  <td className="p-3 text-muted-foreground" colSpan={7}>No emails found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => load(false)} disabled={loading || !cursor}>
