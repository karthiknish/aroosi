...
import { getQuickPicks, actOnQuickPick, QuickPickProfile } from "@/lib/engagementUtil";
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <Card className="w-full max-w-md">
        <CardContent
          ...
          onTouchStart={(e) => (e.currentTarget as any)._touchStartX = e.changedTouches[0].clientX}
          onTouchEnd={(e) => {
            const start = (e.currentTarget as any)._touchStartX as number | undefined;
        ...
        >
          ...
          <div className="text-xs text-gray-500">
            {index + 1} / {userIds.length} shown • Limit {dailyLimit} ({subscription?.plan || "free"})
          </div>
          {currentId ? (
            <div className="flex flex-col items-center gap-4 w-full">
              ...
              <div className="text-center">
                <div className="text-sm font-semibold">
                  <Link href={`/profile/${currentId}`} className="hover:underline">
                    {currentProfile?.fullName || "Member"}
                  </Link>
                </div>
                <div className="text-xs text-gray-600">{currentProfile?.city || ""}</div>
              </div>
              <div className="flex gap-4">
                <Button variant="outline" onClick={() => onAction("skip")}>Skip</Button>
