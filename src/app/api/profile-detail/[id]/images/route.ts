...
function getTokenFromRequest(req: NextRequest): string | null {
  try {
    ...
    if (!auth) {
      debug && console.log('[API] No authorization header found');
      return null;
    }
    const [type, token] = auth.split(" ");
    if (type !== "Bearer" || !token) {
      debug && console.log('[API] Invalid authorization format or missing token');
      return null;
    }
    return token;
  } catch (error) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Define the type for the profile data we'll use
    type ProfileWithUserId = {
      userId: Id<"users">;
    ...
    try {
      ...
      try {
        ...
        if (profile) {
          ...
          log('Found profile by ID', { profileId: id, userId: userId.toString() });
        }
      } catch (profileError) {
        log('Profile not found by ID, trying user ID', { error: String(profileError) });
      }
      ...
      try {
        ...
        throw new Error(`Failed to fetch images: ${queryError instanceof Error ? queryError.message : 'Unknown error'}`);
      }
