...
export async function GET(request: NextRequest) {
  try {
    ...
    const city = searchParams.get("city")?.trim().replace(/[<>'\"&]/g, '') || undefined;
    const country = searchParams.get("country")?.trim().replace(/[<>'\"&]/g, '') || undefined;
    ...
    console.log(`Profile search by user ${userId}: page=${page}, filters=${JSON.stringify({ city: !!city, ageMin, ageMax, preferredGender })}`);
