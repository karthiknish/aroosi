...
export async function GET(request: NextRequest) {
  try {
    ...
    // Rate limiting
    const rateLimitResult = checkApiRateLimit(`get_conversations_${userId}`, 60, 60000); // 60 requests per minute
    ...
    const unreadCounts = await client.query(api.messages.getUnreadCountsForUser, {
    ...
    const conversations = await Promise.all(
      matches.map(async (match: any) => {
        ...
        return {
          ...
          lastActivity: lastMessage?.createdAt || lastMessage?._creationTime || match.createdAt || Date.now(),
          lastMessageAt: lastMessage?.createdAt || lastMessage?._creationTime,
          unreadCount: unreadCounts[match.userId] || 0,
          createdAt: match.createdAt || Date.now(),
          updatedAt: lastMessage?.createdAt || lastMessage?._creationTime || match.createdAt || Date.now(),
        };
      })
    );
