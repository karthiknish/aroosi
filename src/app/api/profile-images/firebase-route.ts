...
export async function POST(request: NextRequest) {
  ...
  await db.collection("users").doc(user.id).collection("images").doc(fileName).set({
          url: publicUrl,
          storageId: path,
          fileName: fileName,
          originalName: file.name,
        ...
        // Handle JSON data
        const { userId, storageId, fileName, contentType, size } = await request.json();
  ...
  await db.collection("users").doc(user.id).collection("images").doc(imageId).set({
          url: imageUrl,
          storageId: storageId,
          fileName: fileName,
...
export async function DELETE(request: NextRequest) {
  ...
  await db.collection("users").doc(user.id).collection("images").doc(fileName).delete();
