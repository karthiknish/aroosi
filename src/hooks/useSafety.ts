...
export const useBlockUser = () => {
  ...
  return useMutation({
    mutationFn: (blockedUserId: string) => safetyAPI.blockUser(blockedUserId),
    onSuccess: (_, blockedUserId) => {
      showSuccessToast('User blocked successfully');
      // Invalidate relevant queries
      queryClient.invalidateQueries({ queryKey: ['blockedUsers'] });
      queryClient.invalidateQueries({ queryKey: ['blockStatus', blockedUserId] });
...
export const useUnblockUser = () => {
  ...
  return useMutation({
    mutationFn: (blockedUserId: string) => safetyAPI.unblockUser(blockedUserId),
    onSuccess: (_, blockedUserId) => {
...
export const useBlockedUsers = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await safetyAPI.getBlockedUsers();
...
export const useBlockStatus = (userId: string) => {
  return useQuery({
    ...
    queryFn: () => safetyAPI.checkBlockStatus(userId),
    enabled: !!userId,
...
export const useSafety = () => {
  ...
  return {
    ...
    reportUser: async (userId: string, reason: ReportReason, description?: string): Promise<boolean> => {
      try {
