...
export function useDeliveryReceipts({ conversationId, token }: UseDeliveryReceiptsProps) {
  ...
  const fetchDeliveryReceipts = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        data.deliveryReceipts?.forEach((receipt: DeliveryReceipt) => {
  ...
  const getMessageDeliveryStatus = useCallback((messageId: string, isCurrentUser: boolean): DeliveryStatus => {
