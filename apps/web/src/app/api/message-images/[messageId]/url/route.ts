...
export const GET = withFirebaseAuth(async (authUser, request: NextRequest) => {
  const correlationId = Math.random().toString(36).slice(2, 10);
  try {
    const userId = authUser.id;
    const url = new URL(request.url);
    const parts = url.pathname.split("/");
    // pathname: /api/message-images/{messageId}/url
    const idx = parts.findIndex((p) => p === "message-images");
    const messageId = idx >= 0 ? parts[idx + 1] : "";
    if (!messageId) {
