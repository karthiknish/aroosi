...
const respondBodySchema = interestsRespondOnlySchema.omit({ interestId: true });

export const POST = createAuthenticatedHandler(
  async (
    ctx: ApiContext,
    body: import("zod").infer<typeof respondBodySchema>
  ) => {
    ...
    if (!interestId || typeof interestId !== "string") {
      return errorResponse("interestId is required", 400, {
    ...
    try {
      ...
      console.error("interests/[interestId]/respond POST error", {
        error,
        correlationId: ctx.correlationId,
      });
  ...
  {
    bodySchema: respondBodySchema,
    rateLimit: {
      identifier: "interests_respond",
      maxRequests: 60,
      windowMs: 600000,
    },
  }
