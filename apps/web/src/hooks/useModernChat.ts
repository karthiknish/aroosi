...
export function useModernChat({
  ...
  useEffect(() => {
    ...
    // Only run if we have a valid matchUserId
    if (!matchUserId) return;

    // Prevent duplicate initialization
    if (presenceInitializedRef.current) return;
    ...
    // Initial fetch - stagger the calls to avoid burst
    (async () => {
      if (!mounted) return;
      try {
        await heartbeat();
    ...
    const initialPresenceTimer = setTimeout(async () => {
      if (!mounted) return;
      try {
        const p = await getPresence(matchUserId);
        if (mounted) setOtherPresence(p);
