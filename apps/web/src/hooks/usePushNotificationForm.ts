...
export function usePushNotificationForm() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      showErrorToast(null, `Failed to ${dryRun ? "preview" : "send"} notification`);
    } finally {
      setSending(false);
    }
  }, [
    ...
    mutableContent, contentAvailable, appliedTemplateId
  ]);
  ...
  const saveCurrentAsTemplate = useCallback(async (name: string, description: string, loadTemplates: () => void) => {
    if (!name.trim()) {
