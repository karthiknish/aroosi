...
export function validateRequestOrigin(request: Request): boolean {
  ...
  if (referer) {
    ...
    const allowedHosts = allowedOrigins.map(url => new URL(url).host);
...
export function logSecurityEvent(
  event: 'RATE_LIMIT_EXCEEDED' | 'INVALID_TOKEN' | 'UNAUTHORIZED_ACCESS' | 'VALIDATION_FAILED',
...
export function validateSecurityRequirements(request: Request): {
  ...
  for (const header of suspiciousHeaders) {
    if (request.headers.get(header)) {
      logSecurityEvent('VALIDATION_FAILED', { reason: `Suspicious header: ${header}` }, request);
