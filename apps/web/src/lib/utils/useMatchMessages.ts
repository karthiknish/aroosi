...
export function useMatchMessages(conversationId: string, token: string) {
  ...
  useEffect(() => {
    ...
    // Use token in URL as fallback for SSE (browsers don't support headers in EventSource)
    // Note: This is less secure but necessary for SSE functionality
    ...
    const connect = () => {
      ...
      es.onerror = (e) => {
        ...
        if (reconnectAttempts < maxReconnectAttempts) {
          ...
          console.log(`[SSE] Attempting reconnection in ${delay}ms (attempt ${reconnectAttempts + 1}/${maxReconnectAttempts})`);
